...
public class GuiTooltips {
    ...
    public static List<net.minecraft.network.chat.Component> VoidMaw(Player player){
        ...
        if (item.isEmpty()) {
            ...
            list.add(net.minecraft.network.chat.Component.translatable("gui.iter.void_maw.tooltip.repaircost",
                    net.minecraft.network.chat.Component.translatable("gui.iter.void_maw.tooltip.explevel"), item.get(DataComponents.REPAIR_COST),
                    (int) Math.max(0, (((item.get(DataComponents.REPAIR_COST)+1)/2)+1))));
        };
        return list;
    }
    ...
    public static boolean VoidMawCondition(Player player){
        ...
        if (item.get(DataComponents.REPAIR_COST) < 7) return false;
