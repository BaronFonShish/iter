...
public class StingerItem extends BowItem {
    ...
    public StingerItem() {
        super(new Properties().rarity(Rarity.COMMON).durability(514));
    }
            ...
            if (!itemstack.isEmpty() || flag) {
                ...
                if (!((double) f < 0.1D)) {
                    ...
                    if (!pLevel.isClientSide) {
                        ...
                        abstractarrow.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, f * m_Velocity, 5.0F);
                        if (f == 1.0F) {
                        ...
                        if (j > 0) {
                            powerBonus = 1 + ((j + 1) * 0.25);
                        }

                        abstractarrow.setBaseDamage(baseDamage * powerBonus * m_Power + f_Power);
