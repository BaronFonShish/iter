...
public class JaggedBeam extends AbstractBeam {
    ...
    public JaggedBeam(Level level, Vec3 startpos, Vec3 endpos, int lifetime, float width, float segment_length, float deviation, float alpha,
                      boolean fading, boolean shrinking, boolean flickering, boolean refresh, String texture) {
    ...
    private List<Vec3> DefineSegments(){
        ...
        for (int i = 0; i <= SegmentAmount; i++){
            ...
            float deviation_scale = -(4 * progress * (progress-1));

            Vec3 offset = new Vec3((this.level().random.nextFloat() - 0.5f) * deviation_scale * deviation,
                                    (this.level().random.nextFloat() - 0.5f) * deviation_scale * deviation,
                                    (this.level().random.nextFloat() - 0.5f) * deviation_scale * deviation);
